/**
 * fuzzer_client.c - Client implementation for NexCache fuzzer
 *
 * This file implements a client that connects to a NexCache server,
...
static int isServerDisconnected(nexcacheContext *ctx) {
    return (ctx->err == NEXCACHE_ERR_EOF ||
            (ctx->err == NEXCACHE_ERR_IO &&
             (errno == ECONNRESET || errno == EPIPE || errno == ENOTCONN)));
}

/* Connect to a NexCache server with optional TLS support
 * Returns a nexcacheContext on success, NULL on error
 * ssl_config can be NULL to disable TLS */
static nexcacheContext *connectToServer(const char *host, int port, cliSSLconfig *ssl_config) {
    nexcacheContext *ctx;
    nexcacheReply *reply;
    ...
    ctx = nexcacheConnectWithOptions(&options);
    if (!ctx) {
        logMessage(LOG_ERROR, "[Thread %d] Connection error: can't allocate nexcache context", thread_id);
        return NULL;
    }
    ...
    nexcacheSetTimeout(ctx, command_timeout);

    /* Establish TLS connection if ssl_config is provided */
    if (ssl_config) {
        const char *tls_err = NULL;
        if (cliSecureConnection(ctx, *ssl_config, &tls_err) == NEXCACHE_ERR && tls_err) {
    ...
    reply = nexcacheCommand(ctx, "PING");
 ...
 * ssl_config can be NULL to disable TLS */
static nexcacheContext *reconnectWithBackoff(const char *host, int port, cliSSLconfig *ssl_config) {
    int attempts = 0;
...
static void handleMalformedReply(nexcacheContext *ctx, FuzzerCommand *cmd) {
...
static CommandResult handleCommandError(nexcacheContext *ctx, FuzzerCommand *cmd, time_t start_time) {
    CommandResult result = CMD_ERROR;

    if (ctx->err == NEXCACHE_ERR_IO && (errno == EAGAIN || errno == EWOULDBLOCK || errno == ETIMEDOUT)) {
    ...
    } else if (ctx->err == NEXCACHE_ERR_PROTOCOL) {
...
static void logReplyDebug(const char *command, nexcacheReply *reply) {
    if (current_log_level < LOG_DEBUG) return;
...
static void validateReplyAndUpdateCounters(nexcacheReply *reply, FuzzerCommand *cmd) {
...
static CommandResult sendCommandAndGetReply(nexcacheContext *ctx) {
    int ret = CMD_SUCCESS;
    ...
    nexcacheReply *reply = nexcacheCommandArgv(ctx, cmd->argc, (const char **)cmd->argv, NULL);
...
/* Connect and fuzz a NexCache server with optional TLS support
 ...
 * This function is called by nexcache-benchmark when fuzz mode is enabled */
