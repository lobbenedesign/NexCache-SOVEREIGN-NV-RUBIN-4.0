...
void lolwut9Command(client *c) {
    ...
    output_array = sdscatprintf(output_array, "Don't forget to have fun! %s ver. ", server.extended_nexcache_compat ? "NexCache" : "NexCache");
    output_array = sdscat(output_array, server.extended_nexcache_compat ? NEXCACHE_VERSION : NEXCACHE_VERSION);
