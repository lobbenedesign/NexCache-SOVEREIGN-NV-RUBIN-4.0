...
NexEngine *engine_create(int num_workers, int port, const char *bind_addr) {
    /* NEX-FIX: NexEngine/NexWorker/mpsc_node_t are declared aligned(256) but
     * calloc guarantees only 16-byte alignment on macOS — a violated alignment
     * attribute is UB for the internal atomic accesses. Allocate aligned. */
