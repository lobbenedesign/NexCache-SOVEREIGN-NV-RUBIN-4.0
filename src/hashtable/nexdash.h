 ...
 *
 * vs Redis dict.c:
 *   - Overhead per entry: ~16 bit vs 64 bit Redis (4x meno memoria)
 *   - Directory overhead quasi zero vs 8N bytes di Redis
 *   - Fork-less snapshot DELTA (Redis richiede fork + 3x memoria)
 *   - Eviction 2Q + ML prediction (Redis usa LRU random sampling)
 *
