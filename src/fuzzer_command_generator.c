...
dict *parseConfigOutput(nexcacheReply *reply) {
    ...
    for (size_t i = 0; i < reply->elements; i += 2) {
        ...
        if (reply->element[i]->type != NEXCACHE_REPLY_STRING ||
            reply->element[i + 1]->type != NEXCACHE_REPLY_STRING) continue;
...
static void processArgumentFlags(CommandArgument *cmdArg, nexcacheReply *flags) {
    static const struct {
        const char *name;
        int flag;
    } flagMap[] = {
...
/* Forward declaration */
static void parseCommandArguments(nexcacheReply *arguments, CommandArgument *result, struct CommandInfo *parent);

static void processSubarguments(CommandArgument *cmdArg, nexcacheReply *arguments) {
...
static void parseCommandArgument(CommandArgument *cmdArg, nexcacheReply *argMap) {
    if (argMap->type != NEXCACHE_REPLY_MAP && argMap->type != NEXCACHE_REPLY_ARRAY) return;

    for (size_t i = 0; i < argMap->elements; i += 2) {
        assert(argMap->element[i]->type == NEXCACHE_REPLY_STRING);
        char *key = argMap->element[i]->str;
        nexcacheReply *value = argMap->element[i + 1];
        ...
        if (!strcmp(key, "name")) {
            ...
            assert(value->type == NEXCACHE_REPLY_STRING);
...
static size_t countTotalCommands(nexcacheReply *commandTable) {
...
static void populateCommandEntry(CommandEntry *command, sds cmdName, sds subcommandName) {
    ...
    nexcache_strlcpy(command->fullname, command->argv[0], fullnameLength);

    if (subcommandName) {
        nexcache_strlcat(command->fullname, " ", fullnameLength);
        nexcache_strlcat(command->fullname, command->argv[1], fullnameLength);
    }
...
static void updateCommand(sds cmdName, int arity, nexcacheReply *flagsArray) {
    for (size_t j = 0; j < fuzz_ctx->commandRegistrySize; j++) {
        ...
        for (size_t k = 0; k < flagsArray->elements; k++) {
            if (flagsArray->element[k]->type != NEXCACHE_REPLY_STATUS) continue;
...
static void extractCommandFlags(nexcacheReply *info) {
    if (!info || info->type != NEXCACHE_REPLY_ARRAY) {
        return;
    }

    /* Iterate through each command in the COMMAND output */
    for (size_t i = 0; i < info->elements; i++) {
        nexcacheReply *cmdEntry = info->element[i];

        /* Each command entry should be an array */
        if (cmdEntry->type != NEXCACHE_REPLY_ARRAY || cmdEntry->elements < 6) continue;

        /* Get command name */
        if (cmdEntry->element[0]->type != NEXCACHE_REPLY_STRING) continue;

        sds cmdName = cmdEntry->element[0]->str;

        /* Get command arity (element 1 in the COMMAND output) */
        int arity = 0;
        if (cmdEntry->element[1]->type == NEXCACHE_REPLY_INTEGER) {
            arity = cmdEntry->element[1]->integer;
        }

        /* Get command flags (element 2 in the COMMAND output) */
        if (cmdEntry->element[2]->type != NEXCACHE_REPLY_ARRAY) continue;

        nexcacheReply *flagsArray = cmdEntry->element[2];
...
static CommandEntry *initializeCommandEntry(sds cmdName, sds subcommandName, CommandEntry *next, nexcacheReply *specs) {
    ...
    for (size_t j = 0; j < specs->elements; j += 2) {
        ...
        if (!strcmp(key, "arguments")) {
            ...
            if (specs->element[j + 1]->type == NEXCACHE_REPLY_STRING) {
...
static void initializeCommandRegistry(nexcacheReply *commandTable) {
...
/* Initialize the fuzzer with a connected NexCache context */
int initFuzzer(nexcacheContext *ctx, int num_keys, int cluster_mode, int fuzz_flags) {
    int ret = -1;
    ...
    nexcacheReply *commandDocs = NULL;
    nexcacheReply *commandInfo = NULL;
    nexcacheReply *configOutput = NULL;
    nexcacheReply *aclCatOutput = NULL;

    /* Execute COMMAND DOCS to get command documentation */
    commandDocs = nexcacheCommand(ctx, "COMMAND DOCS");
    if (!commandDocs || commandDocs->type == NEXCACHE_REPLY_ERROR) {
    ...
    commandInfo = nexcacheCommand(ctx, "COMMAND");
    if (!commandInfo || commandInfo->type == NEXCACHE_REPLY_ERROR) {
    ...
    configOutput = nexcacheCommand(ctx, "CONFIG GET *");
    if (!configOutput || configOutput->type == NEXCACHE_REPLY_ERROR) {
    ...
    aclCatOutput = nexcacheCommand(ctx, "ACL CAT");
    if (!aclCatOutput || aclCatOutput->type == NEXCACHE_REPLY_ERROR) {
...
/* Generate plausible string values for NexCache command arguments */
...
void generateCommandsWithLua(FuzzerCommand *cmd) {
    ...
    for (int i = 0; i < numCommands; i++) {
        ...
        script = sdscatprintf(script, "result[%d] = nexcache.call(", i + 1);
