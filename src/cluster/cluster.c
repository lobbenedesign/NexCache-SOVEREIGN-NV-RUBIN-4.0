 ...
 *      CRC16(key) → slot [0-16383] → nodo (compatibile Redis Cluster).
...
/* ── CRC16 (algoritmo CCITT, compatibile Redis Cluster) ─────── */
...
int cluster_key_to_slot(const char *key, size_t keylen) {
     ...
     * Compatibile con Redis Cluster hash tags. */
