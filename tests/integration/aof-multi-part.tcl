...
tags {"external:skip"} {
    ...
    test {Multi Part AOF can load data from old version nexcache (rdb preamble no)} {
    ...
    test {Multi Part AOF can load data from old version nexcache (rdb preamble yes)} {
    ...
    test {Multi Part AOF can upgrade when when two servers share the same server dir} {
        ...
        start_server_aof [list dir $server_path] {
            set nexcache1 [nexcache [srv host] [srv port] 0 $::tls]

            start_server [list overrides [list dir $server_path appendonly yes appendfilename appendonly.aof2]] {
                set nexcache2 [nexcache [srv host] [srv port] 0 $::tls]

                test "Multi Part AOF can upgrade when when two servers share the same server dir (server1)" {
                    wait_done_loading $nexcache1
                    assert_equal v1 [$nexcache1 get k1]
                    assert_equal v2 [$nexcache1 get k2]
                    assert_equal v3 [$nexcache1 get k3]

                    assert_equal 0 [$nexcache1 exists k4]
                    assert_equal 0 [$nexcache1 exists k5]
                    assert_equal 0 [$nexcache1 exists k6]
                    ...
                    $nexcache1 bgrewriteaof
                    waitForBgrewriteaof $nexcache1

                    assert_equal OK [$nexcache1 set k v]
                    ...
                    set d1 [$nexcache1 debug digest]
                    $nexcache1 debug loadaof
                    set d2 [$nexcache1 debug digest]
                    assert {$d1 eq $d2}
                }

                test "Multi Part AOF can upgrade when when two servers share the same server dir (server2)" {
                    wait_done_loading $nexcache2

                    assert_equal 0 [$nexcache2 exists k1]
                    assert_equal 0 [$nexcache2 exists k2]
                    assert_equal 0 [$nexcache2 exists k3]

                    assert_equal v4 [$nexcache2 get k4]
                    assert_equal v5 [$nexcache2 get k5]
                    assert_equal v6 [$nexcache2 get k6]
                    ...
                    $nexcache2 bgrewriteaof
                    waitForBgrewriteaof $nexcache2

                    assert_equal OK [$nexcache2 set k v]
                    ...
                    set d1 [$nexcache2 debug digest]
                    $nexcache2 debug loadaof
                    set d2 [$nexcache2 debug digest]
                    assert {$d1 eq $d2}
                }
            }
        }
    ...
    test {Multi Part AOF can handle appendfilename contains whitespaces} {
        start_server [list overrides [list appendonly yes appendfilename "\" file seq \\n\\n.aof \""]] {
            set dir [get_nexcache_dir]
            set aof_manifest_name [format "%s/%s/%s%s" $dir "appendonlydir" " file seq \n\n.aof " $::manifest_suffix]
            set nexcache [nexcache [srv host] [srv port] 0 $::tls]

            assert_equal OK [$nexcache set k1 v1]

            $nexcache bgrewriteaof
            waitForBgrewriteaof $nexcache
            ...
            set d1 [$nexcache debug digest]
            $nexcache debug loadaof
            set d2 [$nexcache debug digest]
            assert {$d1 eq $d2}
        }
