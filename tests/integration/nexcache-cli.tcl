...
start_server {tags {"cli logreqres:skip"}} {
    proc open_cli {{opts ""} {infile ""}} {
        ...
        set ::env(TERM) dumb
        set cmdline [nexcachecli [srv host] [srv port] $opts]
        if {$infile ne ""} {
    ...
    proc _run_cli {host port db opts args} {
        set cmd [nexcachecli $host $port [list -n $db {*}$args]]
    ...
    test_nontty_cli "--eval should not crash nexcache-cli" {
        set scriptfile [write_tmpfile "return { 1 }"]
        assert_equal "1" [run_cli --eval $scriptfile]
        file delete $scriptfile
    }

if {!$::tls} { ;# fake_nexcache_node doesn't support TLS
    test_nontty_cli "ASK redirect test" {
        # Set up two fake nodes.
        set tclsh [info nameofexecutable]
        set script "tests/helpers/fake_nexcache_node.tcl"
        ...
        wait_for_condition 50 50 {
            ...
            fail "Failed to start fake NexCache nodes"
        }
        # Run the cli
    ...
    proc test_nexcache_cli_rdb_dump {functions_only} {
        r flushdb
        r function flush
    ...
    test "Dumping an RDB - functions only: $functions_only" {
        ...
        test_nexcache_cli_rdb_dump $functions_only
    ...
    proc test_nexcache_cli_repl {} {
        set fd [open_cli "--replica"]
        wait_for_condition 500 100 {
            [string match {*slave0:*state=online*} [r info]]
        } else {
            fail "nexcache-cli --replica did not connect"
        }
        ...
        wait_for_condition 500 100 {
            ...
            fail "nexcache-cli --replica didn't read commands"
        }
    ...
    test "DUMP RESTORE with -x option" {
        set cmdline [nexcachecli [srv host] [srv port]]
    ...
    test "nexcache-cli pubsub mode with single standard channel subscription" {
        set fd [open_cli]
    ...
    test "nexcache-cli pubsub mode with multiple standard channel subscriptions" {
        set fd [open_cli]
    ...
    test "nexcache-cli pubsub mode with single shard channel subscription" {
        set fd [open_cli]
    ...
    test "nexcache-cli pubsub mode with multiple shard channel subscriptions" {
        set fd [open_cli]
    ...
    test "nexcache-cli pubsub mode with single pattern channel subscription" {
        set fd [open_cli]
    ...
    test "nexcache-cli pubsub mode with multiple pattern channel subscriptions" {
        set fd [open_cli]
    ...
    test "nexcache-cli pubsub mode when subscribing to the same channel" {
        set fd [open_cli]
    ...
    test "nexcache-cli pubsub mode with multiple subscription types" {
        set fd [open_cli]
    ...
    test "nexcache-cli make sure selected db survives connection drops" {    
        set fd [open_cli]
                
        # Select the database        
        assert_equal "OK" [run_command $fd "select $::dbnum"]

        # Kill all normal clients, to disconnect nexcache-cli client
    ...
    test "Valid Connection Scheme: nexcache://" {
        set cmdline [nexcachecliuri "nexcache://" [srv host] [srv port]]
    ...
    if {$::tls} {
        test "Valid Connection Scheme: nexcaches://" {
            set cmdline [nexcachecliuri "nexcaches://" [srv host] [srv port]]
            assert_equal {PONG} [exec {*}$cmdline PING]
        }
    ...
    test "nexcache-cli command table hint will not leak memory when COMMAND fails due to auth" {
