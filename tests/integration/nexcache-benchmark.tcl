...
# common code to reset stats, flush the db and run nexcache-benchmark
proc common_bench_setup {cmd} {
    r config resetstat
    r flushall
    if {[catch { exec {*}$cmd } output]} {
        set first_line [lindex [split $output "\n"] 0]
        puts [colorstr red "nexcache-benchmark non zero code, the output is: $output"]
        fail "nexcache-benchmark non zero code. first line: $first_line"
    }
    return $output
}
...
tags {"benchmark network external:skip logreqres:skip"} {
    start_server {} {
        ...
        test {benchmark: set,get} {
            set cmd [nexcachebenchmark $master_host $master_port "-c 5 -n 10 -t set,get"]
            common_bench_setup $cmd
            default_set_get_checks
        }

        test {benchmark: connecting using URI set,get} {
            set cmd [nexcachebenchmarkuri $master_host $master_port "-c 5 -n 10 -t set,get"]
            common_bench_setup $cmd
            default_set_get_checks
        }
        ...
        test {benchmark: connecting using URI with authentication set,get} {
            ...
            set cmd [nexcachebenchmarkuriuserpass $master_host $master_port "default" pass "-c 5 -n 10 -t set,get"]
            common_bench_setup $cmd
            default_set_get_checks
        }

        test {benchmark: full test suite} {
            set cmd [nexcachebenchmark $master_host $master_port "-c 10 -n 100"]
            common_bench_setup $cmd
        ...
        test {benchmark: multi-thread set,get} {
            set cmd [nexcachebenchmark $master_host $master_port "--threads 10 -c 5 -n 10 -t set,get"]
            common_bench_setup $cmd
            default_set_get_checks
        ...
        test {benchmark: pipelined full set,get} {
            set cmd [nexcachebenchmark $master_host $master_port "-P 5 -c 10 -n 10010 -t set,get"]
            common_bench_setup $cmd
        ...
        test {benchmark: arbitrary command} {
            set cmd [nexcachebenchmark $master_host $master_port "-c 5 -n 150 INCRBYFLOAT mykey 10.0"]
            common_bench_setup $cmd
        ...
        test {benchmark: arbitrary command sequence} {
            set cmd [nexcachebenchmark $master_host $master_port "-n 12 -- incr foo ; 3 incr bar"]
            common_bench_setup $cmd
        ...
        test {benchmark: arbitrary command with data placeholder} {
            set cmd [nexcachebenchmark $master_host $master_port "-n 1 -d 42 -- set k value:__data__"]
            common_bench_setup $cmd
            puts [r get k]
        ...
        test {benchmark: keyspace length} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 50 -t set -n 1000"]
            common_bench_setup $cmd
        ...
        test {benchmark: keyspace covered by sequential option} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 50 -t set -n 50 --sequential"]
            common_bench_setup $cmd
        ...
        test {benchmark: multiple independent sequential replacements} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 50 -n 1000 --sequential -- set j__rand_int__ rain ; set k__rand_1st__ rain"]
            common_bench_setup $cmd
            assert_match  {*calls=1000,*} [cmdstat set]
        ...
        test {benchmark: multiple occurrences of first placeholder have different values} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 100 -n 100 --sequential -- set rain__rand_int__ rain__rand_int__"]
            common_bench_setup $cmd
            assert_match  {*calls=100,*} [cmdstat set]
        ...
        test {benchmark: besides first placeholder, multiple placeholder occurrences have same value} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 100 -n 100 -P 5 --sequential -- set rain__rand_1st__ rain__rand_1st__"]
            common_bench_setup $cmd
            assert_match  {*calls=100,*} [cmdstat set]
        ...
        test {benchmark: multiple placeholder occurrences have same value} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 30000000 -n 20 -- set rain__rand_int__ rain__rand_1st__"]
            common_bench_setup $cmd
        ...
        test {benchmark: sequential zadd results in expected number of keys} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 50 -n 50 --sequential -t zadd"]
            common_bench_setup $cmd
        ...
        test {benchmark: warmup and duration are cumulative} {
            ...
            set cmd [nexcachebenchmark $master_host $master_port "-r 5 --warmup 1 --duration 1 -t set"]
        ...
        test {benchmark: warmup can be used with request count} {
            set start_time [clock clicks -millisec]
            set cmd [nexcachebenchmark $master_host $master_port "-r 5 --warmup 1 -n 100 -t set"]
            set output [common_bench_setup $cmd]
            set end_time [clock clicks -millisec]
        ...
        test {benchmark: -n and --duration are mutually exclusive} {
            set cmd [nexcachebenchmark $master_host $master_port "-r 5 -n 5 --duration 1 -t set"]
        ...
        test {benchmark: warmup applies to all tests in multi-test run} {
            set start_time [clock clicks -millisec]
            set cmd [nexcachebenchmark $master_host $master_port "-r 5 --warmup 2 -n 50 -t set,get,incr"]
            set output [common_bench_setup $cmd]
            set end_time [clock clicks -millisec]
        ...
        test {benchmark: clients idle mode should return error when reached maxclients limit} {
            set cmd [nexcachebenchmark $master_host $master_port "-c 10 -I"]
        ...
        test {benchmark: read last argument from stdin} {
            set base_cmd [nexcachebenchmark $master_host $master_port "-x -n 10 set key"]
        ...
        test {benchmark: CSV output format} {
            set cmd [nexcachebenchmark $master_host $master_port "-c 5 -n 10 -t set,get --csv"]
            set output [common_bench_setup $cmd]
        ...
        test {benchmark: quiet mode} {
            set cmd [nexcachebenchmark $master_host $master_port "-c 5 -n 10 -t set,get -q"]
            set output [common_bench_setup $cmd]
        ...
        if {$::tls} {
            test {benchmark: specific tls-ciphers} {
                set cmd [nexcachebenchmark $master_host $master_port "-r 50 -t set -n 1000 --tls-ciphers \"DEFAULT:-AES128-SHA256\""]
                common_bench_setup $cmd
                assert_match  {*calls=1000,*} [cmdstat set]
                # assert one of the non benchmarked commands is not present
                assert_match  {} [cmdstat get]
            ...
            test {benchmark: specific tls-ciphersuites} {
                ...
                set cmd [nexcachebenchmark $master_host $master_port "-r 50 -t set -n 1000 --tls-ciphersuites \"TLS_AES_128_GCM_SHA256\""]
                if {[catch { exec {*}$cmd } error]} {
                    ...
                    if {[string match "*Invalid option*" $first_line]} {
                        ...
                        puts [colorstr red "nexcache-benchmark non zero code. first line: $first_line"]
                        fail "nexcache-benchmark non zero code. first line: $first_line"
                    }
                }
