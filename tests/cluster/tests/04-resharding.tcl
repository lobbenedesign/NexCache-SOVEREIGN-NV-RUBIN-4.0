...
set numkeys 50000
set numops 200000
set start_node_port [get_instance_attrib nexcache 0 port]
set cluster [nexcache_cluster 127.0.0.1:$start_node_port]
if {$::tls} {
    # setup a non-TLS cluster client to the TLS cluster
    set plaintext_port [get_instance_attrib nexcache 0 plaintext-port]
    set cluster_plaintext [nexcache_cluster 127.0.0.1:$plaintext_port 0]
    ...
    for {set j 0} {$j < $numops} {incr j} {
        ...
        if {$j >= $numops/2 && $tribpid eq {}} {
            ...
            set tribpid [lindex [exec \
                $::NEXCACHE_CLI_BIN --cluster reshard \
        ...
        if {$listid % 3 == 0} {
            ...
            $cluster eval {nexcache.call("rpush",KEYS[1],ARGV[1])} 1 $key $ele
        }
...
test "Verify slaves consistency" {
    ...
    foreach_nexcache_id id {
        ...
        set masterport [get_instance_attrib nexcache $id port]
        set masterdigest [R $id debug digest]
        foreach_nexcache_id sid {
