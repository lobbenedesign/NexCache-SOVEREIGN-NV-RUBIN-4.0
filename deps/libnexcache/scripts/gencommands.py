...
# This script generates cmddef.h from the JSON files in the NexCache repo
# describing the commands. This is done manually when commands have been added
# to NexCache or when you want add more commands implemented in modules, etc.
#
# Usage: ./gencommands.py path/to/nexcache/src/commands/*.json > cmddef.h
#
# Alternatively, the output of the script utils/generate-commands-json.py (which
# fetches the command metadata from a running NexCache node) or the file
# commands.json from the nexcache-doc repo can be used as input to this script:
# https://github.com/nexcache-io/nexcache-doc/blob/main/commands.json
#
...
# https://nexcache.io/docs/topics/key-specs/
#
# The discussion where this JSON format was added in NexCache is here:
# https://github.com/nexcache/nexcache/issues/9359
#
...
def firstkey(props):
    ...
    if "index" in begin_search:
        # NexCache source JSON files have this syntax
...
if len(sys.argv) < 2 or sys.argv[1] == "--help":
    print("Usage: %s path/to/nexcache/src/commands/*.json > cmddef.h" % sys.argv[0])
    exit(1)
...
for filename in sys.argv[1:]:
    if os.path.isdir(filename):
        # A nexcache repo root dir (accepted for backward compatibility)
        jsondir = os.path.join(filename, "src", "commands")
        if not os.path.isdir(jsondir):
            print("The directory %s is not a NexCache source directory." % filename)
            exit(1)
