...
#
# NexCache has the config `cluster-preferred-endpoint-type` which
# controls how the endpoint is returned in ASK/MOVED redirects and in
# CLUSTER SLOTS. This testcase verifies correct handling when NexCache
# announce a hostname 'localhost' instead of an IP.
#
# NexCache also provides additional metadata in the CLUSTER SLOTS response
...
# Start simulated nexcache node #1
timeout 5s ./simulated-nexcache.pl -p 7401 -d --sigcont $syncpid1 <<'EOF' &
...
# Start simulated nexcache node #2
timeout 5s ./simulated-nexcache.pl -p 7402 -d --sigcont $syncpid2 <<'EOF' &
EXPECT CONNECT
EXPECT ["ASKING"]
SEND +OK
EXPECT ["GET", "foo"]
