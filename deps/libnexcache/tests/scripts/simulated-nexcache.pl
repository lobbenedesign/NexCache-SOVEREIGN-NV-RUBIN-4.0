...
# A tool for simulating the traffic of a NexCache node.
#
...
while ($_ = shift) {
    if (/^(?:-p|--port)$/) {
        ...
        map { print "$_\n" }
            "Usage: $0 [ OPTIONS ]",
            "",
            "Acts as a NexCache node, communicating with clients according to",
            ...
            "escaped NexCache protocol data, optionally without the final '\\r\\n'.",
            "Examples:",
            "",
            "  SEND +OK",
            "  SEND -ERR Some error",
...
while (<>) {
    ...
    if (/^SEND (.*)/) {
        my $data = $1;
        if ($data =~ /^[-+\$\*:]/) {
            # NexCache protocol with character escapes
            # e.g. '-ERR Unknown command: FOO\r\n'
            $data = unescape($data);
            $data .= "\r\n" unless $data =~ /\r\n$/;
        } else {
            # e.g. '["foo", "bar", 42]'
            $data = nexcache_encode(JSON->new->allow_nonref->decode($1));
        }
