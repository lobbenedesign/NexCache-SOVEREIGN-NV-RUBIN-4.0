...
void printReply(const nexcacheReply *reply) {
    switch (reply->type) {
...
void replyCallback(nexcacheClusterAsyncContext *acc, void *r, void *privdata) {
    ...
    if (reply == NULL) {
        ...
        if (resend_failed_cmd) {
            printf("resend '%s'\n", cmd_history[cmd_id]);
            if (nexcacheClusterAsyncCommand(acc, replyCallback, (void *)cmd_id,
                                          cmd_history[cmd_id]) != NEXCACHE_OK)
                printf("send error\n");
            return;
        }
    } else {
...
void sendNextCommand(evutil_socket_t fd, short kind, void *arg) {
    UNUSED(fd);
    UNUSED(kind);
    nexcacheClusterAsyncContext *acc = arg;
    int async = 0;
    ...
    while (fgets(cmd, CMD_SIZE, stdin)) {
        ...
        if (send_to_all) {
            ...
            while ((node = nexcacheClusterNodeNext(&ni)) != NULL) {
                int status = nexcacheClusterAsyncCommandToNode(
            ...
            int status = nexcacheClusterAsyncCommand(
...
int main(int argc, char **argv) {
    ...
    if (use_cluster_nodes) {
        options.options |= NEXCACHE_OPT_USE_CLUSTER_NODES;
    }
