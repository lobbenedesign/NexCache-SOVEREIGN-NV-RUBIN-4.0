...
void connectCallback(const nexcacheContext *c, int status) {
    const char *s = "";
    ...
    while (fgets(command, 256, stdin)) {
        ...
        if (send_to_all) {
            ...
            while ((node = nexcacheClusterNodeNext(&ni)) != NULL) {
                nexcacheReply *reply =
                    nexcacheClusterCommandToNode(cc, node, command);
                if (!reply || cc->err) {
                    printf("error: %s\n", cc->errstr);
                } else {
                    printReply(reply);
            ...
            nexcacheReply *reply = nexcacheClusterCommand(cc, command);
            if (!reply || cc->err) {
                printf("error: %s\n", cc->errstr);
            } else {
                printReply(reply);
