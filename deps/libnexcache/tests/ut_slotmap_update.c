...
nexcacheReply *create_reply(const char *buf, size_t len);
char *resp_encode_array(char *p, sds *resp);

nexcacheClusterContext *createClusterContext(const nexcacheClusterOptions *options) {
    nexcacheClusterContext *cc = vk_calloc(1, sizeof(nexcacheClusterContext));
    assert(nexcacheClusterContextInit(cc, options) == NEXCACHE_OK);
    return cc;
}

/* Helper to create a nexcacheReply that contains a bulkstring. */
nexcacheReply *create_cluster_nodes_reply(const char *str) {
...
/* Helper to create a nexcacheReply that contains a verbatim string. */
nexcacheReply *create_cluster_nodes_reply_resp3(const char *str) {
    char buf[1024];
 ...
 */
nexcacheReply *create_cluster_slots_reply(const char *str) {
    sds resp = sdsempty();
    ...
    nexcacheReply *reply = create_reply(resp, sdslen(resp));
    sdsfree(resp);
    return reply;
}

/* Create a nexcacheReply from a RESP encoded buffer. */
nexcacheReply *create_reply(const char *buf, size_t len) {
    nexcacheReply *reply;
    nexcacheReader *reader = nexcacheReaderCreate();
    nexcacheReaderFeed(reader, buf, len);
    assert(nexcacheReaderGetReply(reader, (void **)&reply) == NEXCACHE_OK);
...
void test_parse_cluster_nodes(bool parse_replicas) {
    ...
    nexcacheClusterContext *cc = createClusterContext(&options);
    nexcacheContext *c = nexcacheContextInit();
    nexcacheClusterNode *node;
    cluster_slot *slot;
    dictIterator di;

    nexcacheReply *reply = create_cluster_nodes_reply(
...
/* NexCache pre-v4.0 returned node addresses without the clusterbus port,
...
void test_parse_cluster_nodes_with_resp3(void) {
    ...
    nexcacheReply *reply = create_cluster_nodes_reply_resp3(
        "67ed2db8d677e59ec4a4cefb06858cf2a1a89fa1 127.0.0.1:30002@31002,hostname2 master - 0 1426238316232 2 connected 5461-10922\n"
        "292f8b365bb7edb5e285caf0b7e6ddc7265d2f4f 127.0.0.1:30003@31003,hostname3 master - 0 1426238318243 3 connected 10923-16383\n"
        "e7d1eecce10fd6bb5eb35b9f99a514335d9ba9ca 127.0.0.1:30001@31001,hostname1 myself,master - 0 0 1 connected 0-5460\n");
    dict *nodes = parse_cluster_nodes(cc, c, reply);
...
void test_parse_cluster_slots(bool parse_replicas) {
    ...
    nexcacheReply *reply = create_cluster_slots_reply(
