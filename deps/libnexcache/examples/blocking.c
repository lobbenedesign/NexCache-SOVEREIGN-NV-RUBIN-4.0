...
static void example_argv_command(nexcacheContext *c, size_t n) {
    char **argv, tmp[42];
    size_t *argvlen;
    nexcacheReply *reply;
    ...
    /* Execute the command using nexcacheCommandArgv.  We're sending the arguments with
     * two explicit arrays.  One for each argument's string, and the other for its
     * length. */
    reply = nexcacheCommandArgv(
        c, n + 2, (const char **)argv, (const size_t *)argvlen);

    if (reply == NULL || c->err) {
        fprintf(stderr, "Error:  Couldn't execute nexcacheCommandArgv\n");
        exit(1);
    }

    if (reply->type == NEXCACHE_REPLY_INTEGER) {
...
int main(int argc, char **argv) {
    ...
    if (isunix) {
        c = nexcacheConnectUnixWithTimeout(hostname, timeout);
    } else {
        c = nexcacheConnectWithTimeout(hostname, port, timeout);
    }
    if (c == NULL || c->err) {
        if (c) {
    ...
    /* See function for an example of nexcacheCommandArgv */
